...
# Use ALL unique nodes (no slicing)
...
# Build LSTM model (slightly larger for more nodes, but still CPU-friendly)
model = Sequential([
...
print("Training on all nodes... (this may take several minutes)")
...
# Forecast for ALL nodes (using last sequence of each node)
forecasts = {}
for node, group in df.groupby('node_id'):
    values = group['energy_norm'].values
    if len(values) >= sequence_length:
        ...
        forecasts[node] = float(values[-1] * scaler.data_range_[0] + scaler.min_[0]) if len(values) > 0 else 50.0
