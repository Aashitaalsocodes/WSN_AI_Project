"""
routing_cost_final.py - LSTM with BATCH prediction (works on Windows)
"""
...
RESULT_PATH = OUTPUTS / f"routing_cost_results_seed{SEED}_lstm_batch.json"
...
class BatchLSTMEnergyRouter:
    """LSTM router with BATCH prediction (no deadlock)."""
    ...
    def load_model(self):
        try:
            ...
            if model_path.exists():
                ...
                print("  [OK] LSTM model loaded (batch mode)")
                return True
            else:
...
def main():
    print("\n" + "=" * 60)
    print("ROUTING COST ENGINE (LSTM BATCH MODE)")
    print("=" * 60)
    print(f"Seed: {SEED}")
    print("-" * 60)
    ...
    # LSTM router
    print("Initializing LSTM (batch mode)...")
    lstm_router = BatchLSTMEnergyRouter()
    ...
    # First pass: collect energy history
    print("Collecting energy history for LSTM...")
    for nid in node_ids:
        lstm_router.update_history(nid, node_feat[nid]["energy_risk"])
    
    # BATCH predict (ONE call, not 200)
    print("Running LSTM batch prediction...")
    lstm_router.batch_predict_all()
    print(f"  [OK] Predicted {len(lstm_router.forecasts)} nodes")
    ...
    def edge_cost(u, v, edge_attrs):
        ...
        # LSTM penalty (from batch prediction)
    ...
    with open(RESULT_PATH, "w") as f:
        json.dump({"routes": results, "summary": {"pct_compromised": pct, "avg_hops": avg_hops, "lstm_enabled": True}}, f, indent=2)
